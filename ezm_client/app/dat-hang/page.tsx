...
function OrderContent() {
	...
	const handleOrder = async () => {
		try {
			...
			if (order && productParam && variantParam) {
				items = [
					{ variantId: order.variants.itemVariantId, quantity: order.quantity },
				];
			} else {
				items = cart.map((item) => ({
			...
			if (response?.data?.status !== 0) {
				const message = response?.data?.errors?.message || 'Đặt hàng thất bại';
	...
	const total =
		order && productParam && variantParam
			...
			: cart.reduce(
					(total, item) => total + Number(item.variants?.price) * item?.count,
					0
			  );
					...
					<div className="flex-grow mt-6 lg:mt-0">
									...
									<div>
										...
										<div className="variants">
											<span className="px-3 bg-ezman-red text-white">
												{order?.variants?.item_variant_item_size_fk
													? order?.variants?.item_variant_item_size_fk?.name
													: ''}
												{order?.variants?.item_variant_item_color_fk
						...
						<div className="w-full mt-4 space-y-2">
							...
							<div className="text-gray-500 flex justify-between items-center">
								...
								<span className="font-semibold text-lg">
									{Number(0).toLocaleString()}đ
								</span>
							</div>
							<div className="flex justify-between items-center">
								<span className="font-semibold text-lg">Tổng tiền</span>
								<span className="font-semibold text-lg">
									{Number(total + 30000).toLocaleString()}đ
								</span>
							</div>

							<button
								onClick={handleOrder}
								className="mt-2 cursor-pointer bg-ezman-red w-full py-2 text-lg font-medium text-white"
							>
								Đặt hàng
							</button>
						</div>
...
function OrderPage() {
	return (
		<Suspense
			fallback={<div className="px-4 lg:px-12 text-white">Đang tải...</div>}
		>
			<OrderContent />
		</Suspense>
	);
